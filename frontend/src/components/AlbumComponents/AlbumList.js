...
const AlbumList = ({ albums, handleAlbumSelect, handleAlbumCreate, handleAlbumDelete }) => {
    ...
    return (
        <>
            ...
            {albums.map((album) => (
                <li
                    onClick={() => {handleAlbumSelect(album.id); setDisplayDelete(album.id); setChooseAlbum(album.id);}}
                    key={album.id}
